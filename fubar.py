# -*- coding: utf-8 -*-
"""
Created on Wed Jun 24 15:24:41 2020
...
file = 'fubar_passeri40_1.xls' #Reads the first file
with pd.ExcelFile(file) as xls:
    for sheet_name in xls.sheet_names:
        df = pd.read_excel(file, sheet_name='PALM') #Reads the sheet chosen
        #print(df.head())
   
#When the values from column Prob[alpha<beta] are over 0.95, get the value from column 'Site' in the same row     
pss_1 = df.loc[df['Prob[alpha<beta]'] > 0.95, 'Site'] 
#print(pss_1)
...
#When the values from column Prob[alpha>beta] are equal or over 0.95, get the value from column 'Site' in the same row     
nss_1 = df.loc[df['Prob[alpha>beta]'] > 0.95, 'Site']     
#print(nss_1)

nss_list_1 = list(nss_1) #Converts int64 into list
print('1st Sites:', nss_list_1) #Prints the NSS sites in the first file
print('1st Number of NSS sites:', len(nss_list_1)) #Prints the number of NSS sites in the first file



file = 'fubar_passeri40_1.xls' #Reads the second file
with pd.ExcelFile(file) as xls:
    for sheet_name in xls.sheet_names:
        df = pd.read_excel(file, sheet_name='fubar_passeri40_1') #Reads the sheet chosen
        #print(df.head())
 
#When the values from column Prob[alpha<beta] are over 0.95, get the value from column 'Site' in the same row            
pss_2 = df.loc[df['Prob[alpha<beta]'] > 0.95, 'Site']       
#print(pss)
...
#When the values from column Prob[alpha>beta] are equal or over 0.95, get the value from column 'Site' in the same row     
nss_2 = df.loc[df['Prob[alpha>beta]'] > 0.95, 'Site']     
#print(nss)

nss_list_2 = list(nss_2) #Converts int64 into list
print('2nd Sites:', nss_list_2) #Prints the number of NSS sites in the second file
print('2nd Number of NSS sites:', len(nss_list_2)) #Prints the number of NSS sites in the second file




file = 'fubar_passeri40_1.xls' #Reads the third file
with pd.ExcelFile(file) as xls:
    for sheet_name in xls.sheet_names:
        df = pd.read_excel(file, sheet_name='fubar_passeri40_1') #Reads the sheet chosen
        #print(df.head())
...
#When the values from column Prob[alpha<beta] are over 0.95, get the value from column 'Site' in the same row            
pss_3 = df.loc[df['Prob[alpha<beta]'] > 0.95, 'Site']    
#print(pss)

pss_list_3 = list(pss_3) #Converts int64 into list
print('3rd Sites:', pss_list_3) #Prints the number of NSS sites in the second file
print('3rd Number of PSS sites:', len(pss_list_3))


nss_3 = df.loc[df['Prob[alpha>beta]'] > 0.95, 'Site']      
#print(nss)

nss_list_3 = list(nss_3) #Converts int64 into list
print('3rd Sites:', nss_list_3)
print('3rd Number of NSS sites:', len(nss_list_3))
...
'''Compare the lists and get a final list for NSS'''
